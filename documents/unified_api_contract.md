...
> 注：好友申请的前提条件（对方须在 `chat_history` 中）仅由客户端校验，服务端不做此检查。
...
**频率：** 每 5 秒一次，仅向 `ble_find` 列表中的设备发送。
...
| `chatable` | 实时计算 `ble_find ∪ heartbeat_receive` | 最终的"附近可聊天"列表。离开范围 ≤1 分钟内仍可聊天；超过 1 分钟临时会话过期 |
| `chat_history` | 设备进入 `chatable` 时自动追加 | 所有曾经 chatable 的用户（持久化，不因离开而删除）。好友申请前提：对方在此列表中（仅客户端校验） |
...
- 本地维护 `ble_find`、`heartbeat_receive`、`chatable`、`chat_history`、`friend`、`block` 六个列表
- 通过 `/presence/resolve` 将 BLE 扫描结果转换为用户信息
- 消息优先走 WebSocket，断线降级为 HTTP POST
- 心跳保活：WebSocket ping 每 30 秒一次；BLE heartbeat 每 5 秒一次（仅向 ble_find 发送）
